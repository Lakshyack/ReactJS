...
export default function TextForm(props) {
  ...
  return (
    ...
    <div className='container text-center text-dark'>
         ...
         <button disabled={text.length === 0} className='btn btn-success my-3 mx-1 my-1' onClick={handleLoClick}>Convert to LowerCase</button>
         <button disabled={text.length === 0} className='btn btn-danger my-3 mx-1 my-1' onClick={handleClearClick}>Clear Text</button>
    </div>
  ...
  <p style={{ color: props.mode === 'dark' ? 'white' : '#202020' }}>
    {text.split(/\s+/).filter((element) => element.length !== 0).length} words and {text.split("").filter((char) => char !== ' ').length} Characters
  </p>
  <p style={{ color: props.mode === 'dark' ? 'white' : '#202020' }}>
